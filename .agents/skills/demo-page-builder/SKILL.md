...
- **用户明确认可后（"可以了/满意/就这样"），打 git tag 并生成与 tag 同名的交接文档**（`docs/handover/<tag名>.md`，描述产品经理希望用户如何使用这个页面、在网页上可以做什么，给出页面 DOM 树并说明每个组件是什么，对话过程摘要放最后），完整流程见 `references/handover.md`；这是交付的收尾动作，不可省略
