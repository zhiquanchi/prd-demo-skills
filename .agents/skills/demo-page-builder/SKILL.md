...
复刻场景的其余规则（白名单、git 提交、交付确认）与常规任务完全一致——用户确认满意后同样打 git tag 并生成交接文档，交接文档要求（使用说明 + 用户操作流程图 + DOM 树与组件说明 + 对话过程摘要）与新增页面一视同仁。
...
- **用户明确认可后（"可以了/满意/就这样"），打 git tag 并生成与 tag 同名的交接文档**（`docs/handover/<tag名>.md`，**新增页面与复刻 demo 均适用**，描述使用者实际使用这个页面的操作逻辑并配用户操作流程图、给出页面 DOM 树并说明每个组件是什么、对话过程摘要放最后），完整流程见 `references/handover.md`；这是交付的收尾动作，不可省略
