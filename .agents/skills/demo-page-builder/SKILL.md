...
| 可交互元素实现规范、元素与行为解耦（同一行为收敛为单一实现）、「交互不生效」四层排查 | `references/interactivity.md` |
...
- **一条迭代消息含多个改动点时逐项拆条目**：用户一次要求改多个元素（交互、样式、文案等，如「把这 8 个卡片都改成可点击」）时，先按「交付与确认 → 业务改动」的批量改动规则枚举改动清单，**每个改动点一条「实现 + git 提交」条目**，禁止合并成一条模糊条目整体处理
...
- subagent 完成后，主会话必须检查文件边界、白名单、mock 契约和构建结果，**并逐项核对派发任务里列出的每个改动点是否全部落地**（源码里确认对应代码存在，漏任何一项即不合格）。
...
- **批量改动先拆清单再动手（强制，防「部分生效」）**：一条用户消息里含 2 个及以上改动点（如同时调整多个元素的交互性）时，动手前先把每个改动点枚举成带编号的改动清单，每项写清「哪个元素/文件 + 改成什么行为 + 怎么验证」，逐项登记进 `todo_tool`（与开工清单条目 4 同粒度，一项一条）；实现、生效、汇报全部按清单逐项推进。**禁止把整批当成一个模糊任务整体处理**——凭记忆顺手改最容易漏掉其中几项，用户看到的就是「部分生效、部分没生效」。**同类交互涉及多个元素时优先按 `references/interactivity.md`「元素与行为解耦」把行为收敛为单一实现**（元素侧只留绑定声明），把 N 处改动收敛为 1 处，从架构上消灭漏改面。
- **逐项核对，禁止以点代面（强制）**：页面级 `verify-page --marker` 通过只证明「页面进了产物」，**不证明每个改动点都生效**（单一 marker 命中时其余改动可能根本没写进代码）。批量改动报完成前必须输出逐项核对表（编号 → 改动点 → 验证方式 → 结果）：改交互的项在源码里核对每项的事件处理器/状态/props 确实存在且已挂到对应元素上；能提炼 ASCII 特征串的改动每项一个独立 marker；关键交互用浏览器逐项实际操作一遍。任何一项未通过，先按 `references/interactivity.md` 的「四层排查」（源码→构建→服务→浏览器）定位到层再修，修复、重新生效后重发地址。
