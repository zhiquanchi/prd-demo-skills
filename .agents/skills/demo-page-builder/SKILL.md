...
- 用户确认后，在原有 `docs/handover/<tag名>.md` 上**就地更新**——以当前代码为准重写五个章节、全部适用功能用例与**一一对应的用户流程（3.2 每功能一条，功能有增改就同步增补/更新）**，不在新增文档。自检全 `[PASS]` 后必须 `git add -A && git commit`（提交信息 `docs: 更新 <tag名> 交接文档`），再打一个新的 `deliver-*` tag 标记本次修订代码的版本——**有改动就有提交，文档与代码改动都不允许停留在未提交状态**。
