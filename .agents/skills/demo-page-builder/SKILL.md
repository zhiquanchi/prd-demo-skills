...
- 修改前先用 `git log --oneline --all -- <页面路径> <相关公共组件路径>` 和 `git tag -l 'deliver-*'` 定位该页面最近的交付 tag 与文档；布局/公共组件变更必须列出所有受影响页面。
- 新改动生效后，仍按本节先下发访问地址并等待用户确认。确认前只提交业务改动，**不得改写旧 tag 或旧交接文档来伪造最新状态**。
- 用户确认后，为每个受影响页面创建**新的** `deliver-<页面语义名>-<日期>-<序号>` tag 和同名完整交接文档；新文档必须基于当前代码重写全部五个章节，并在「五、对话过程摘要」首条注明“交付修订：继承 `<上一tag>`，本次变更为 …”。
- 旧 tag 与旧文档是历史快照，保持不变；“最新交接文档”定义为该页面最近一次 `deliver-*` tag 对应的文档。详细步骤与核对规则见 `references/handover.md` 的「已交付页面的后续修改」。
...
3. **识别交付修订（硬性）**：本次若修改了已交付页面，先定位上一交付 tag；新文档按当前代码完整重写，并在「对话过程摘要」记录继承关系与本次修改，禁止只补一段 diff 或覆盖旧文档
4. **按模板生成**：以 `handover.template.md` 为骨架，保持五个一级章节不变，每个功能填全 0–8 用例字段；**一份文档只写一个页面**——多页面交付按页面拆成多份、逐份生成逐份校验
