...
- 用户确认后，在原有 `docs/handover/<tag名>.md` 上**就地更新**——以当前代码为准重写五个章节与全部适用功能用例，不在新增文档。自检全 `[PASS]` 后必须 `git add -A && git commit`（提交信息 `docs: 更新 <tag名> 交接文档`），再打一个新的 `deliver-*` tag 标记本次修订代码的版本——**有改动就有提交，文档与代码改动都不允许停留在未提交状态**。
...
3. **识别交付修订（硬性）**：本次若修改了已交付页面，先定位上一交付 tag；已有对应交接文档则就地更新原文件，没有则新建——基于当前代码完整重写五个章节与全部功能用例，并在「对话过程摘要」中追加修订记录
