...
function main() {
  ...
  // 提取 DOM 树代码块（``` 包裹的树状结构）
  const treeLines = extractTreeLines(domLines);
  if (treeLines.length === 0) {
    check(false, 'DOM 树存在且非空', undefined);
  } else {
    check(true, 'DOM 树存在且非空', undefined);
...
/** 提取 DOM 树代码块中的树状行（排除说明/注释行） */
function extractTreeLines(domLines) {
  const inFence = [];
  let collecting = false;
  for (const ln of domLines) {
    const fence = ln.trim().match(/^```/);
    if (fence) { collecting = !collecting; continue; }
    if (collecting) inFence.push(ln);
  }
  return inFence
    .filter((ln) => {
      const t = ln.trim();
      if (!t) return false;
      if (/^#/.test(t)) return false;          // 注释
      return /[│├└]/.test(t);                  // 含树形符号即视为树行
    });
}
