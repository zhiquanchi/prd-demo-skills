...
> 「3.2 用户流程」与「功能说明」的用例一一对应：**功能有几个，就写几条用户流程**（每条含主流程/分支/异常）；页面功能后续更新或新增时，同步增补/更新对应流程。
...
<!--
  ...
  粒度硬性要求（前端实现依据）：细化到字段/控件级——分组区块逐组展开，每个可交互控件单独成叶子，
  叶子行必须带 # 用途注释（字段名/按钮名、必填、校验、options 枚举、placeholder、关键 props）。
-->

```
<页面名> (src/pages/<页面组件>/index.tsx)                     # 项目页面组件：<职责>
├── <区块一>
│   └── <项目组件A> (src/components/<组件A>/index.tsx)      # 项目组件：<职责>
│       └── <库>:<容器组件>                                   # 容器：<布局与关键 props，如 disabled 策略>
│           ├── <分组组名一>
│           │   ├── <库>:<控件组件>                           # <字段名>：必填/选填，<校验规则>，placeholder「<文案>」
│           │   ├── <库>:<控件组件>                           # <字段名>：必填，选项 <option value 枚举>
│           │   └── <库>:<控件组件>                           # <字段名>：valuePropName=checked，<默认值>
│           ├── <分组组名二>
│           │   └── <库>:<控件组件>                           # <按钮名>：<type/htmlType> 与点击行为
│           └── <库>:<原子组件>                                # <用途>（如组间分隔 Divider，共 <N> 条）
└── <区块二>
    └── <库>:<原子组件>            # 无独立项目组件时直接用原子组件：<用途注释>
```
...
<!-- 前端还原依据：把原型里看不出或容易做错的布局规则写清楚 -->
- 内容区宽度与居中：<如 max-width 920px 水平居中>
- 栅格与分栏：<如桌面双列 grid、列间距 20px、开关行占满整行>
- 响应式断点：<如 ≤700px 改单列、padding 收窄>
- 禁用与反馈：<如 loading/saving 时表单整体禁用、提交按钮 loading>
- 关键色值与间距：<如卡片边框 #ececf1、辅助文字 #9893a9>
...
### 3.2 用户流程（User Flow）
<!-- 本页有几个功能，就写几条用户流程，每条与「四、功能说明」的用例一一对应（功能名称与用例编号一致）：
     功能更新或新增后，同步增补/更新对应流程。每条流程用文字或 Mermaid 描述「开始 → 步骤 → 决策点 → 结束」，
     并列关键分支与异常/退出路径；开头可加一条「总体流程」串起所有功能，再按功能逐一展开。 -->
#### 总体流程（可选）
- 主流程（Happy Path）：<进入页面 → 完成各项操作 → 结束>
- 关键分支路径：
  - 路径 A：<分支说明>
  - 路径 B：<分支说明>
- 异常/退出路径：<异常/退出说明>

#### 用户流程：<功能名称>（<用例编号，如 UC-XXX-01>）
