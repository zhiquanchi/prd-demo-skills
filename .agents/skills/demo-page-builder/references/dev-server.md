...
运行本 skill 的环境探测脚本，看输出里的 `environment=`（初始化阶段一般已跑过；POSIX 用 `scripts/posix/check-environment.sh`，Windows 用 `scripts/windows/check-environment.ps1`；不提供 cmd `.bat` 版本，cmd 环境请改用 PowerShell）：`wsl` → 模式 A，`native` → 模式 B。等价于手动执行：
...
Windows（无 bash）用对应的 PowerShell 版脚本（输出与退出码一致）：
...
不提供 cmd `.bat` 版本，cmd 环境请改用 PowerShell。
...
- **症状→排查映射：页面骨架正常、接口请求后弹出「XXX 数据加载失败，请刷新重试」类错误提示**（页面 `.catch()` 兜底的 message.error 文案，不是白屏）：说明页面与构建基本没问题，是接口 404/异常触发了兜底。按顺序排查：
  1. 看运行中服务的启动日志有无该接口的 Mock API 行（`Mock API: /api/<name> -> mock/<name>.json`；模式 B 无此日志，直接进第 3 步）；
  2. **日志里没有 → 新增 mock 未重启**：`serve-dist.js` 的路由表只在启动时注册，按「重启前必须检测端口占用」停掉旧进程、重启服务，再用 `--api` 重新断言；
  3. **日志里有但请求仍 404 → 查三处一致性**：`mock/<name>.json` 文件是否存在、`mock/<domain>.ts` 导出的接口 key、页面 `request` 的路径（`mock/<name>.json` ⇄ `/api/<name>` ⇄ `request('/api/<name>')`），任何一处对不上都会 404。

## 已知现象（两种模式通用）

- 首页路径 `/` 返回 200 只说明服务活着，不说明页面有内容。
- 没有任何 `src/pages` 时，根路径是空白页（返回的 HTML 只有空的 `<div id="root">`），这是正常的。
- 页面骨架正常、接口请求后才弹「数据加载失败，请刷新重试」类提示：不是页面代码问题，是 mock 接口 404 触发了 `.catch()` 兜底——按「服务管理」的症状→排查映射定位，最常见原因是新增 mock 后没重启服务。
